...
Default constructors, copy constructors, copy assignment operators, move constructors, move assignment operators, and prospective destructors are the special member functions. These functions are automatically generated by the compiler if they are used, but not declared explicitly by the programmer.

The compiler generated versions of these functions perform a memberwise operation (e.g., the compiler generated destructor will destroy each member or base class of the object). The compiler generated functions are `public`, non-virtual, and the copy constructor and copy assignment operators will receive `const&` parameters.
